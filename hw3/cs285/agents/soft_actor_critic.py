...
class SoftActorCritic(nn.Module):
    ...
    def q_backup_strategy(self, next_qs: torch.Tensor) -> torch.Tensor:
        ...
        if self.target_critic_backup_type == "doubleq":
            assert next_qs.shape[0] == 2, "Double-Q requires exactly two critic networks"
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        # sample_size = 300
        # torch_sample_size = torch.Size((sample_size,))
        sampled_actions = action_distribution.rsample()
        entropy = -action_distribution.log_prob(sampled_actions)
        return entropy
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            q_values = self.critic(obs.unsqueeze(0).expand(self.num_actor_samples, -1, -1).contiguous(), action)
        ...
        mean_entropy = torch.mean(self.entropy(action_distribution))
    ...
    def update_actor(self, obs: torch.Tensor):
        ...
        ################## DOMANDA: questa quantità non dovrebbe essere moltiplicata 
        # per il logaritmo della probabilità della policy?
    ...
    def update(
        ...
        for i in range(self.num_critic_updates):
            if i != self.num_critic_updates - 1:
