...
class DQNAgent(nn.Module):
    ...
    def get_action(self, observation: np.ndarray, epsilon: float = 0.02) -> int:
        ...
        # TODO check whether this is only one observation or they are multiple

        # TODO(student): get the action from the critic using an epsilon-greedy strategy
        with torch.no_grad():
            qa_values = self.critic(observation)
            if np.random.rand() < epsilon:
                action = np.random.randint(self.num_actions, size=(1,))
            else:
                action = qa_values.argmax(dim=-1).cpu().numpy()
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            if self.use_double_q:
                ...
                next_action = next_qa_values_online.argmax(dim=-1)
            else:
    ...
    def update(
        ...
        update_info = self.update_critic(obs, action, reward, next_obs, done)

        if step % self.target_update_period == 0:
            self.update_target_critic()
            update_info["target_update"] = True
