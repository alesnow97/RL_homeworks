...
def collect_mbpo_rollout(
    ...
    for _ in range(rollout_len):
        ...
        next_ob = next_ob.mean(axis=0).squeeze()
    ...
    return {
        ...
        "action": np.array(acs),
        "reward": np.array(rewards).reshape(-1),
    ...
    for itr in range(config["num_iters"]):
        ...
        for _ in tqdm.trange(
            ...
            for i in range(mb_agent.ensemble_size):
                ...
                current_agent_loss = mb_agent.update(
                    i,
                    batch["observations"],
                    batch["actions"],
                    batch["next_observations"],
                )
                step_losses.append(current_agent_loss)
        ...
        if sac_config is not None:
            ...
            for i in tqdm.trange(
                ...
                sac_agent.update(
                    ...
                    ptu.from_numpy(batch["dones"].astype(bool)),
                    i,
                )
