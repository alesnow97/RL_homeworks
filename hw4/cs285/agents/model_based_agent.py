...
class ModelBasedAgent(nn.Module):
    ...
    def update(self, i: int, obs: np.ndarray, acs: np.ndarray, next_obs: np.ndarray):
        ...
        obs_acs = (obs_acs - self.obs_acs_mean) / (self.obs_acs_std + 1e-6)
        deltas = next_obs - obs
        deltas = (deltas - self.obs_delta_mean) / (self.obs_delta_std + 1e-6)

        loss = self.loss_fn(model(obs_acs), deltas)
    ...
    def update_statistics(self, obs: np.ndarray, acs: np.ndarray, next_obs: np.ndarray):
        ...
        self.obs_delta_mean = deltas.mean(dim=0)
        self.obs_delta_std = deltas.std(dim=0)
    ...
    def get_dynamics_predictions(
        ...
        if len(acs.shape) == 1:
            acs = acs.unsqueeze(0)
        pred_next_obs_delta = model(torch.cat([obs, acs], dim=1))

        pred_next_obs = obs + (pred_next_obs_delta * (self.obs_delta_std + 1e-6) + self.obs_delta_mean)
    ...
    def evaluate_action_sequences(self, obs: np.ndarray, action_sequences: np.ndarray):
        ...
        for acs in action_sequences.transpose(1, 0, 2):
            ...
            # CHANGE, I will use obs instead of next_obs
            rewards, _ = self.env.get_reward(
                obs.reshape(-1, self.ob_dim),
    ...
    def get_action(self, obs: np.ndarray):
        ...
        if self.mpc_strategy == "random":
            ...
            for i in range(self.cem_num_iters):
                if i != 0:
                    action_sequences = np.stack([np.random.multivariate_normal(elite_mean[i], np.diag(elite_std[i]), size=self.mpc_num_action_sequences) for i in range(self.mpc_horizon)], axis=1)
            ...
            return elite_mean[0] if elite_mean is not None else None
        else:
