...
def run_training_loop(params):
    ...
    for itr in range(params['n_iter']):
        ...
        if itr == 0:
            ...
            paths, envsteps_this_batch = utils.sample_trajectories(env, 
                actor, params['batch_size'], params['ep_len'])
            ...
            if params['do_dagger']:
                ...
                for i in range(len(paths)):
                    ...
                    paths[i]["action"] = expert_policy.get_action(obs=ob)
        ...
        for _ in range(params['num_agent_train_steps_per_iter']):
          ...
          num_data = params['train_batch_size']
          indices = np.random.permutation(len(replay_buffer))[:num_data]

          if replay_buffer.obs is None or replay_buffer.acs is None:
              raise ValueError("Replay buffer is empty. Cannot sample data for training.")
